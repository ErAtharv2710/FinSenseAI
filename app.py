...
try:
    ...
    cred_path = "firebase/finsenseai-834cf-firebase-adminsdk-fbsvc-e8ef115421.json"
...
# # Firebase configuration using Pyrebase (Client SDK)
# firebase_config = {
#     "apiKey": os.getenv('FIREBASE_API_KEY', "AIzaSyCJNlKOrGxt19Ewv_MTH9XLb-JPlEOZMq"),
#     "authDomain": os.getenv('FIREBASE_AUTH_DOMAIN', "finsenseai-834cf.firebaseapp.com"),
#     "projectId": os.getenv('FIREBASE_PROJECT_ID', "finsenseai-834cf"),
...
#     "messagingSenderId": os.getenv('FIREBASE_MESSAGING_SENDER_ID', "1089787327198"),
#     "appId": os.getenv('FIREBASE_APP_ID', "1:1089787327198:web:a847d5d61df1444849c84f"),
#     "databaseURL": ""  # Add if you use Realtime Database
# }
...
# def get_user_data(user_id):
#     """Fetch user data from memory (replace with Firebase in production)."""
#     if user_id in user_data:
#         return user_data[user_id]
#     else:
#         # Create default user entry if not exists
...
# def update_user_data(user_id, data):
#     """Update user data in memory (replace with Firebase in production)."""
#     if user_id in user_data:
#         user_data[user_id].update(data)
#     else:
#         user_data[user_id] = data
...
#     current_entertainment_spending = sum(item['amount'] for item in log if item['category'] == 'entertainment')
#     return {'food': current_food_spending, 'entertainment': current_entertainment_spending}

# def check_for_ai_nudge(user):
#     spending = calculate_current_spending(user)
#     food_limit = user['budget_limit']['food']
#     entertainment_limit = user['budget_limit']['entertainment']
    
#     nudges = []
    
#     if spending['food'] > food_limit * 0.50:
#         nudges.append({
#             'message': f"⚠ Nudge! You've spent ₹{spending['food']} on food, "
#                       f"which is halfway to your ₹{food_limit} limit! Try the 'Cook at Home' challenge this week.",
#             'type': 'warning'
#         })
    
#     if spending['entertainment'] > entertainment_limit * 0.75:
#         nudges.append({
#             'message': f"⚠ Alert! Entertainment spending is at ₹{spending['entertainment']} "
#                       f"({spending['entertainment']/entertainment_limit*100:.0f}% of your ₹{entertainment_limit} limit).",
#             'type': 'danger'
#         })
...
#             'xp': 0,
#             'net_worth': 0,
#             'budget_limit': {'food': 5000, 'entertainment': 2000, 'savings': 3000},
#             'spending_log': []
#         }
        
#         # Store in memory for demo (replace with Firebase in production)
...
#         'net_worth': user['net_worth'],
#         'budget_limit': user['budget_limit'],
#         'spending_log': user['spending_log'],
#         'food_progress': food_progress,
#         'entertainment_progress': entertainment_progress,
#         'spending': spending,
#         'nudge': nudge
